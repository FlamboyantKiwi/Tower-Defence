#Tower Defence: Lesson 6: Constant Enemies
import pygame, sys
pygame.init()
pygame.font.init()
...
class EnemySpawner:
    def __init__(self, game_manager):
        ...
        self.spawn_timer = Timer(SPAWN_DELAY, start_active=True)

    def update(self):
        # The timer.update() method returns True only on the exact frame it finishes
        if self.spawn_timer.update():
            self.spawn_enemy()
            self.spawn_timer.activate() # Reset for next enemy

    def spawn_enemy(self):
        # Just spawn a standard enemy for now - will increase difficulty later
        self.manager.create_enemy(ENEMY_HP, ENEMY_SPEED, ENEMY_BOUNTY)
