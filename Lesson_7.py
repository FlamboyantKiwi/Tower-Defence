...
STARTING_ENEMIES = 5 # number of enemies we start with (wave 1)
ENEMIES_PER_WAVE = 2 # Add 2 extra enemies every wave
ENEMY_HP_INCREASE = 5 # Add 5 HP to enemies every wave
...
class EnemySpawner:
    def __init__(self, game_manager):
        ...
        self.state = "COUNTDOWN" #States (starts with countdown)
    ...
    def update(self):
        ...
        if self.state == "COUNTDOWN":
            if self.wave_timer.update(): # Timer finished?
                self.start_new_wave() # Start the wave

        # LOGIC 2: Currently spawning enemies for this wave
        ...
        # Waiting for player to kill everyone (or for enemies to escape)
            ...
            # Check if all enemies are dead or gone
            if len(self.manager.enemies) == 0:
                # Wave cleared! Start countdown again
                self.state = "COUNTDOWN" # begin again
